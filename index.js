...
var start, isBlink, isLight, isRun, isShow, isWarned, handler, latency, stopBy, delay, audioRemind, audioEnd, newAudio, soundToggle, show, adjust, toggle, reset, blink, count, run, resize, format3Digits;
start = null;
isBlink = false;
isLight = true;
isRun = false;
...
// 格式化為 3 位數秒數 (例如 5000ms -> "005")
format3Digits = function(ms) {
  if (ms < 0) ms = 0;
...
adjust = function(it, v){
  ...
  $('#timer').text(format3Digits(delay));
  return resize();
};
...
count = function(){
  ...
  if (diff < 0 && !isBlink) {
    soundToggle(audioEnd, true); // 播放音效
    $('#rocket').addClass('launching'); // 觸發火箭發射
...
window.onload = function(){
  ...
  audioRemind = null; 
  // 請確保您已將火箭音效檔案命名為 rocket_launch.mp3 並放入 audio 資料夾
